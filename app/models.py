...
class FacialExpressionModel:
    ...
    def predict(self, img, items_skimmed_from_top: int = 1):
        global session
        set_session(session)

        preds = self._model.predict(img)
        results = sorted(zip(FacialExpressionModel.EMOTIONS, np.array(preds)[0]), key=lambda item: item[1])
        return results[-items_skimmed_from_top::-1]
